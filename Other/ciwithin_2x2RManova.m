...
% one way anova ile aynı sonucu veriyor şu anda...
...
% TİLL NOW İT WAS LİKE DOİNG ONE WAY RM ANOVA
% now, multiply the MSE with 2 (the number of levels of the second factor)
MSE = 2*MSE;
...
for i=1:r
    CILower_m =  [CILower_m, mean(matrix(:,i))-(tcrit2.*(sqrt(MSE/(N*r/2))))]; 
    CIHigher_m =  [CIHigher_m, mean(matrix(:,i))+(tcrit2.*(sqrt(MSE/(N*r/2))))];  
end
